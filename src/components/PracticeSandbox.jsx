...
/**
 * Interactive Practice Sandbox — multiple-choice daily challenge.
 * Click handlers update an answer state; submitting evaluates correctness and
 * shows a dynamic green "Correct! +50 XP" or red "Try Again" overlay.
 */
  ...
  return (
    ...
    >
      ...
      <div className="p-5 sm:p-6 space-y-5">
        ...
        {/* Options */}
        <div className="grid sm:grid-cols-2 gap-3" role="radiogroup" aria-label="Answer choices">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={[
              'btn-primary',
              (!selected || status === 'correct') && 'opacity-50 cursor-not-allowed',
            ].filter(Boolean).join(' ')}
          >
            {t('submitBtn')}
          </button>
          ...
          <div className="ml-auto text-xs opacity-70">
            {t('xp')}: <span className="font-bold text-ethio-green">{xp.toLocaleString()}</span>
          </div>
        </div>

        {/* Result overlay */}
        {status === 'correct' && <ResultOverlay type="correct" xp={xp - 50} label={t('correctLabel')} />}
        {status === 'wrong' && <ResultOverlay type="wrong" label={t('wrongLabel')} hint={dailyQuestion.explanation[lang]} />}
      </div>

      {/* tiny keyframe for wrong-answer wiggle (kept inline to avoid CSS bloat) */}
      <style>{`
        @keyframes wiggle {
